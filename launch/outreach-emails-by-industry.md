# 雇用主向けアウトリーチ英文メール — 業種別テンプレート

> 🔴 **2026-09-08 訂正あり。** このファイルの「applications come straight to your inbox」
> という記述は**事実ではなくなっていた**（4箇所）。2026-09-04 に応募者の個人情報をメールから
> 外し、ログインの内側に移したため（`src/app/api/apply/route.ts:82` "Deliberately no
> applicant details."）。**仕様を変えたのに、それを説明する文章を変えていなかった。**
> 送っていれば、企業は来ないメールを待つことになっていた。全4箇所を実装に合わせて修正済み。
>
> 📌 **営業の方針・順番・言ってはいけないことは `employer-outreach-kit.md` が正。**
> 本ファイルは文面のバリエーション集として使う。


お付き合いのある3業種（ホテルリゾート / 飲食 / ジェットスキー・ウォータースポーツ）向けに、
業種ごとに文面をカスタムした英文メールです。`[ ]` の部分を差し替えて使ってください。

- 汎用テンプレ（Version A / B）は `employer-outreach-email.md` にあります。本ファイルは業種特化版です。
- 1社ずつ宛名を変えて個別送信すると返信率が上がります（一斉送信は避ける）。
- 返信で求人情報が届いたら、CEOエージェントに転送すれば当日中にサイトへ反映します。

各テンプレの末尾に「掲載に必要な情報リスト」を付けています。相手はこれに返信するだけで掲載できます。

---

## 1. ホテル・リゾート業界向け

**Subject:** Free job listing for [Hotel/Resort Name] — Guam Job Listings 🌴

Hi [Name],

I've just launched **Guam Job Listings** (www.guamjoblisting.com), a new job board
built specifically for Guam's hospitality industry — resorts, hotels, and the teams
that keep our island's guests happy.

Hiring dependable front desk, housekeeping, F&B and guest-services staff is one of
the hardest parts of running a property here, so I'd love to feature
**[Hotel/Resort Name]** as one of our first partners. Posting is **completely free**,
and you're emailed the moment someone applies. Your HR team reads the application —
name, contact details and all — by signing in to the dashboard, because we keep
applicants' personal information on the site rather than in email.

If you have any openings, just reply with the details below and I'll have them live
within a day.

Best regards,
[Your Name]
Guam Job Listings
www.guamjoblisting.com

**To post a job, please reply with:**
- Company / property name
- Position title (e.g. Front Desk Agent, Housekeeping, Concierge)
- Pay / hourly rate
- Location (e.g. Tumon)
- Full-time / Part-time / Seasonal
- A short description of the role and any requirements
- The email address where you'd like to receive applications

---

## 2. 飲食（レストラン・バー）業界向け

**Subject:** Hiring cooks or servers? Free listing on Guam Job Listings 🍽️

Hi [Name],

I've just launched **Guam Job Listings** (www.guamjoblisting.com), a local job board
made for Guam businesses — and restaurants and bars are exactly the kind of employer
we built it for.

Good kitchen and floor staff are hard to find on the island, so I'd love to help
**[Restaurant Name]** get in front of local job seekers. Posting is **completely free**.
Candidates apply in one click, you get an email straight away, and you read the
application by signing in — we keep applicants' personal details off email on purpose.

If you're hiring line cooks, servers, bartenders, dishwashers — anything — just reply
with the details below and I'll get it posted within a day.

Best,
[Your Name]
Guam Job Listings
www.guamjoblisting.com

**To post a job, please reply with:**
- Restaurant / bar name
- Position title (e.g. Line Cook, Server, Bartender)
- Pay / hourly rate (note if tips apply)
- Location (e.g. Tamuning)
- Full-time / Part-time
- A short description of the role and any requirements
- The email address where you'd like to receive applications

---

## 3. ジェットスキー・ウォータースポーツ・ツアー業界向け

**Subject:** Free crew listing for [Company Name] — Guam Job Listings 🌊

Hi [Name],

I've just launched **Guam Job Listings** (www.guamjoblisting.com), a new job board for
Guam employers — with a whole category dedicated to **Water Sports & Tours**, since
that's such a big part of what makes the island special.

Seasonal crew, tour guides and boat staff can be tough to recruit, so I'd love to
feature **[Company Name]** as one of our first tour operators. Posting is **completely
free**, you're emailed as soon as someone applies, and you read the application by
signing in — applicants' personal details stay on the site, not in email.
Bilingual applicants (English / Japanese / Korean) tend to browse the board too.

If you're looking for guides, deckhands or seasonal crew, just reply with the details
below and I'll have your listing live within a day.

Best,
[Your Name]
Guam Job Listings
www.guamjoblisting.com

**To post a job, please reply with:**
- Company name
- Position title (e.g. Jet Ski Tour Guide, Deckhand, Boat Crew)
- Pay / hourly rate (note if tips apply)
- Location (e.g. Piti)
- Full-time / Part-time / Seasonal
- A short description of the role and any requirements (e.g. swimmer, age 18+)
- The email address where you'd like to receive applications

---

## フォローアップ（掲載完了の報告）

掲載後の「載せました」報告は、汎用テンプレの **Version B**（`employer-outreach-email.md`）を
そのまま使えます。件名・本文の `[ ]` を差し替えるだけです。

## 使い方メモ
- 送信前に、相手の会社名・担当者名・想定職種を1社ずつ確認して差し替える。
- 返信のハードルを下げるため「必要情報リスト」への箇条書き返信でOKと伝えている。
- 反応の良い業種が分かったら、トップページでその業種を強調するなど調整する。
